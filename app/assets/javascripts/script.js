...
function initAutocomplete() {
    ...
    if (window.location.search!="" && window.location.search.match(/sou=[^\&^\#]*/g)!=null && window.location.search.match(/sou=[^\&^\#]*/g)[0].split("=")[1]!=""){

        jQuery(document).ready(function(){

            info["referred_by"]=window.location.search.match(/sou=[^\&^\#]*/g)[0].split("=")[1];         
        });
...
function switchScreen(scrno, obj){
    ...
    switch(scrno){
		...
		case 10:
            ...
            html += '<button type="button" class="shuttl-pass text-capitalize paynow btn btn-default col-xs-6 setHeight centerVertical centerHorizontal passtype_1" onclick="choosePass(1);" data-value="1"><div>'+(info.pricing[0][3]!=null?info.pricing[0][3]:18)+' rides @ <span class="strike">'+info.pricing[0][0]+'</span><span class="offer_price">&nbsp;'+info.pricing[0][1]+'</span></div></button>';
            html += '<button type="button" class="shuttl-pass text-capitalize paynow btn btn-default col-xs-6 setHeight centerVertical centerHorizontal passtype_2" onclick="choosePass(2);" data-value="2"><div>'+(info.pricing[1][3]!=null?info.pricing[1][3]:40)+' rides @ <span class="strike">'+info.pricing[1][0]+'</span><span class="offer_price">&nbsp;'+info.pricing[1][1]+'</span></div></button></div>';
...
function fillWhatsAppLink(){
    ...
    jQuery('#whatsapp').attr("href_send","whatsapp://send?text="+"We can save minimum half of our office travel cost. AC buses are available at just Rs 3 per Km. Suggest your home and office location and Shuttl will start the route: http://myor.shuttl.com/suggest/index?paths="+encodedPoints+"&utm_source=whatsapp");
    ...
    jQuery.ajax({url:"/suggest/getWhatsAppShareLink?phone_number="+info.phone_number+"&url="+"http://myor.shuttl.com/suggest/index?paths="+encodedPoints}).done(function(result){
        ...
        jQuery('#whatsapp').attr("href_send","whatsapp://send?text=We can save minimum half of our office travel cost. AC buses are available at just Rs 3 per Km. Suggest your home and office location and Shuttl will start the route: "+url);
